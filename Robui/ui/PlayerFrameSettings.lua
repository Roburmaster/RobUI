local AddonName, ns = ...
local R = _G.Robui
ns.UnitFrames.Player.Settings = ns.UnitFrames.Player.Settings or {}
local UI = ns.UnitFrames.Player.Settings

-- 1. TILGANG TIL CORE
local function PF() return ns.UnitFrames.Player end
local function CFG() return PF() and PF().GetConfig and PF():GetConfig() or nil end
local function Apply()
    local pf = PF()
    if pf and pf.ForceUpdate then pf:ForceUpdate() end
end
...
local TEXTURES = {
    ...
    { name = "Class: Death Knight", path = MEDIA_PATH .. "robui_statusbar_deathknight_256x32.tga" },
    { name = "Class: Demon Hunter", path = MEDIA_PATH .. "robui_statusbar_demonhunter_256x32.tga" },
    { name = "Class: Druid",        path = MEDIA_PATH .. "robui_statusbar_druid_256x32.tga" },
    { name = "Class: Evoker",       path = MEDIA_PATH .. "robui_statusbar_evoker_256x32.tga" },
    { name = "Class: Hunter",       path = MEDIA_PATH .. "robui_statusbar_hunter_256x32.tga" },
    { name = "Class: Mage",         path = MEDIA_PATH .. "robui_statusbar_mage_256x32.tga" },
    { name = "Class: Monk",         path = MEDIA_PATH .. "robui_statusbar_monk_256x32.tga" },
    { name = "Class: Paladin",      path = MEDIA_PATH .. "robui_statusbar_paladin_256x32.tga" },
    { name = "Class: Priest",       path = MEDIA_PATH .. "robui_statusbar_priest_256x32.tga" },
    { name = "Class: Rogue",        path = MEDIA_PATH .. "robui_statusbar_rogue_256x32.tga" },
    { name = "Class: Shaman",       path = MEDIA_PATH .. "robui_statusbar_shaman_256x32.tga" },
    { name = "Class: Warlock",      path = MEDIA_PATH .. "robui_statusbar_warlock_256x32.tga" },
    { name = "Class: Warrior",      path = MEDIA_PATH .. "robui_statusbar_warrior_256x32.tga" },
...
function UI:Build()
    ...
    if not CFG() then
        print("|cffff4444[RobUI]|r Settings not ready.")
        return
    end

    local f = CreateFrame("Frame", "RobUI_PlayerFrame_Settings", UIParent, "BackdropTemplate")
    self.frame = f
    f:SetSize(860, 800)
    f:SetPoint("CENTER")
    Skin(f, C_BG)
    ...
    title:SetPoint("LEFT", 10, 0); title:SetText("RobUI PlayerFrame Settings (Shift+Drag)")
    ...
    self.cbHideAll  = Check(content, "Hide All Elements (Only BG)", x+420, y, nil, function(v) CFG().hideAllElements = v end)
    y = y - 40

    -- B. POSITION
    y = CreateSection(content, "Position (Shift + Left Drag to move)", x, y, W)
    self.rows = {}
    ...
    self.rows.pipH   = SliderRow(content, "Pips Height",   "pipH",     5,   50, 1, x+10, y, 240); y = y - rowH
    self.rows.gap    = SliderRow(content, "Gap Spacing",   "gap",      0,   30, 1, x+10, y, 240); y = y - rowH

    -- D. TEXTURE & COLORS
    y = CreateSection(content, "Bar Colors & Texture", x, y, W)
    Label(content, "Bar Texture:", x+10, y+10)
    local dd = CreateFrame("Frame", "RobUI_PF_TexDD", content, "UIDropDownMenuTemplate")
    self.texDD = dd
    ...
    UIDropDownMenu_Initialize(dd, function()
        ...
        for _,t in ipairs(TEXTURES) do
            UIDropDownMenu_AddButton({
                text = t.name,
                checked = (cfg.texturePath == t.path),
                func = function()
                    cfg.texturePath = t.path
                    cfg.useTexture = true
    ...
    self.cbUseTex = Check(content, "Use Texture", x+300, y-14, nil, function(v) CFG().useTexture=v end)
    self.cbNoTint = Check(content, "Force White (No Tint)", x+420, y-14, nil, function(v) CFG().noColorOverride=v end)
    y = y - 50

    -- Colors HP
    Label(content, "Health Color:", x+10, y+10)
    self.cbClass = Check(content, "Use Class Color", x+10, y-10, nil, function(v) CFG().useClassColor=v; if v then CFG().useCustomHP=false end end)
    self.cbCustom = Check(content, "Use Custom Color", x+200, y-10, nil, function(v) CFG().useCustomHP=v; if v then CFG().useClassColor=false end end)
    Button(content, "Pick HP Color", 120, 22, x+400, y-10, function()
        local cfg = CFG()
        OpenColorPicker(cfg.hpR or 0.2, cfg.hpG or 0.8, cfg.hpB or 0.2, 1, function(r,g,b)
            cfg.hpR, cfg.hpG, cfg.hpB = r, g, b
            cfg.useCustomHP = true; cfg.useClassColor = false
    ...
    self.cbPowCustom = Check(content, "Use Custom Power Color", x+200, y-10, nil, function(v) CFG().useCustomPower=v end)
    Button(content, "Pick Power Color", 120, 22, x+400, y-10, function()
        local cfg = CFG()
    ...
    self.cbPowT = Check(content, "Show Power Text", x+450, y, nil, function(v) CFG().showPowerText=v end)
    self.cbPips = Check(content, "Show Pips", x+600, y, nil, function(v) CFG().pipEnabled=v end)
    y = y - 35
    
    self.cbInc = Check(content, "Incoming Heal (Green)", x+10, y, nil, function(v) CFG().showIncomingHeals=v end)
    self.cbHA  = Check(content, "Heal Absorb (Red)", x+220, y, nil, function(v) CFG().showHealAbsorb=v end)
    self.cbAbs = Check(content, "Shields (Blue)", x+420, y, nil, function(v) CFG().showAbsorb=v end)
    y = y - 50
    ...
    self.rows.powSize = SliderRow(content, "Size", "powerSize", 8, 40, 1, x+10, y-10, 180)
...
function UI.RefreshIfOpen()
    ...
    -- Checkboxes
    UI.cbShown:SetChecked(cfg.shown)
    UI.cbLocked:SetChecked(cfg.locked)
    UI.cbHideAll:SetChecked(cfg.hideAllElements)

    UI.cbName:SetChecked(cfg.showName)
    UI.cbHP:SetChecked(cfg.showHPText)
    UI.cbPow:SetChecked(cfg.showPower)
    UI.cbPowT:SetChecked(cfg.showPowerText)
    UI.cbPips:SetChecked(cfg.pipEnabled)

    UI.cbInc:SetChecked(cfg.showIncomingHeals)
    UI.cbHA:SetChecked(cfg.showHealAbsorb)
    UI.cbAbs:SetChecked(cfg.showAbsorb)

    UI.cbUseTex:SetChecked(cfg.useTexture)
    UI.cbNoTint:SetChecked(cfg.noColorOverride)
    UI.cbClass:SetChecked(cfg.useClassColor)
    UI.cbCustom:SetChecked(cfg.useCustomHP)
    if UI.cbPowCustom then UI.cbPowCustom:SetChecked(cfg.useCustomPower) end

    if UI.texDD then
        UIDropDownMenu_SetText(UI.texDD, NameForPath(cfg.texturePath))
    end

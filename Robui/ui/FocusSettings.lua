local AddonName, ns = ...
local R = _G.Robui
ns.UnitFrames.Focus.Settings = ns.UnitFrames.Focus.Settings or {}
local UI = ns.UnitFrames.Focus.Settings

-- 1. TILGANG TIL CORE
local function Focus() return ns.UnitFrames.Focus end
local function CFG() return Focus() and Focus().GetConfig and Focus():GetConfig() or nil end
local function Apply()
    local f = Focus()
    if f and f.ForceUpdate then f:ForceUpdate() end
end
...
function UI:Build()
    ...
    if not CFG() then
        print("|cffff4444[RobUI]|r Focus settings not ready.")
        return
    end

    local f = CreateFrame("Frame", "RobUI_Focus_Settings", UIParent, "BackdropTemplate")
    self.frame = f
    f:SetSize(860, 820)
    f:SetPoint("CENTER")
    Skin(f, C_BG)
    ...
    title:SetText("RobUI Focus Settings (Shift+Drag window)")
    ...
    -- B. POSITION
    y = CreateSection(content, "Position (Shift + Left Drag focus frame)", x, y, W)
    self.rows = {}
    ...
    local dd = CreateFrame("Frame", "RobUI_Focus_BaseTexDD", content, "UIDropDownMenuTemplate")
    self.baseTexDD = dd
    ...
    self.cbClass = Check(content, "Use Class Color (player focus)", x+10, y-10, nil, function(v)
        local cfg = CFG()
        cfg.useClassColor = v

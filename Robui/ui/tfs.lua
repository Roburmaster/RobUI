local AddonName, ns = ...
local R = _G.Robui
ns.UnitFrames.Target.Settings = ns.UnitFrames.Target.Settings or {}
local UI = ns.UnitFrames.Target.Settings

-- 1. TILGANG TIL CORE
local function TF() return ns.UnitFrames.Target end
local function CFG() return TF() and TF().GetConfig and TF():GetConfig() or nil end
local function Apply()
    local tf = TF()
    if tf and tf.ForceUpdate then tf:ForceUpdate() end
end
...
local TEXTURES = {
    ...
    -- Class previews (faste i targetframe)
    ...
    -- NPC previews (faste i targetframe)
...
function UI:Build()
    ...
    if not CFG() then
        print("|cffff4444[RobUI]|r TargetFrame settings not ready.")
        return
    end

    local f = CreateFrame("Frame", "RobUI_TargetFrame_Settings", UIParent, "BackdropTemplate")
    self.frame = f
    f:SetSize(860, 840)
    f:SetPoint("CENTER")
    Skin(f, C_BG)
    ...
    title:SetText("RobUI TargetFrame Settings (Shift+Drag window)")
    ...
    -- B. POSITION
    y = CreateSection(content, "Position (Shift + Left Drag on targetframe)", x, y, W)
    self.rows = {}
    ...
    local dd = CreateFrame("Frame", "RobUI_TF_BaseTexDD", content, "UIDropDownMenuTemplate")
    self.baseTexDD = dd
    ...
    self.cbClass = Check(content, "Use Class Color on player targets", x+10, y-10, nil, function(v)
        local cfg = CFG()
        cfg.useClassColor = v
        if v then cfg.useCustomHP = false end
    end)
    self.cbCustom = Check(content, "Use Custom HP Tint", x+250, y-10, nil, function(v)
        local cfg = CFG()
        cfg.useCustomHP = v
        if v then cfg.useClassColor = false end
    end)
    Button(content, "Pick HP Tint", 120, 22, x+460, y-10, function()
        local cfg = CFG()
    ...
    self.cbLvl   = Check(content, "Show Level", x+620, y, nil, function(v) CFG().showLevel = v end)
    y = y - 35

    self.cbTag   = Check(content, "Show Tag (Elite/Rare)", x+10, y, nil, function(v) CFG().showClassTag = v end)
    self.cbInc   = Check(content, "Incoming Heal (Green)", x+220, y, nil, function(v) CFG().showIncomingHeals = v end)
    self.cbHA    = Check(content, "Heal Absorb (Red)", x+450, y, nil, function(v) CFG().showHealAbsorb = v end)
    self.cbAbs   = Check(content, "Shields (Blue)", x+650, y, nil, function(v) CFG().showAbsorb = v end)
    y = y - 50
    ...
    -- Level text
    Label(content, "Level Text:", x+10, y+10)
    self.rows.lvlSize = SliderRow(content, "Size", "lvlSize", 8, 40, 1, x+10, y-10, 180)
    self.rows.lvlOffX = SliderRow(content, "Offset X", "lvlOffX", -200, 200, 1, x+10, y-60, 180)
    self.rows.lvlOffY = SliderRow(content, "Offset Y", "lvlOffY", -100, 100, 1, x+400, y-60, 180)
    Button(content, "Color", 60, 22, x+400, y-8, function()
        local cfg = CFG()
        OpenColorPicker(cfg.lvlTextR or 1, cfg.lvlTextG or 1, cfg.lvlTextB or 1, 1, function(r,g,b)
            cfg.lvlTextR, cfg.lvlTextG, cfg.lvlTextB = r, g, b
            Apply()
        end)
    end)
    y = y - 110
    ...
    -- Tag text
    Label(content, "Tag Text (Elite/Rare):", x+10, y+10)
    self.rows.tagSize = SliderRow(content, "Size", "tagSize", 8, 40, 1, x+10, y-10, 180)
    self.rows.tagOffX = SliderRow(content, "Offset X", "tagOffX", -200, 200, 1, x+10, y-60, 180)
    self.rows.tagOffY = SliderRow(content, "Offset Y", "tagOffY", -100, 100, 1, x+400, y-60, 180)
    Button(content, "Color", 60, 22, x+400, y-8, function()
        local cfg = CFG()
        OpenColorPicker(cfg.tagTextR or 1, cfg.tagTextG or 1, cfg.tagTextB or 1, 1, function(r,g,b)
            cfg.tagTextR, cfg.tagTextG, cfg.tagTextB = r, g, b
            Apply()
        end)
    end)
...
function UI.RefreshIfOpen()
    ...
    if UI.cbClass then UI.cbClass:SetChecked(cfg.useClassColor) end
    ...
    if UI.cbLvl then UI.cbLvl:SetChecked(cfg.showLevel) end
    if UI.cbTag then UI.cbTag:SetChecked(cfg.showClassTag) end

local AddonName, ns = ...
local R = _G.Robui
ns.UnitFrames.TargetTarget.Settings = ns.UnitFrames.TargetTarget.Settings or {}
local UI = ns.UnitFrames.TargetTarget.Settings

-- 1. TILGANG TIL CORE
local function ToT() return ns.UnitFrames.TargetTarget end
local function CFG() return ToT() and ToT().GetConfig and ToT():GetConfig() or nil end
local function Apply()
    local tot = ToT()
    if tot and tot.ForceUpdate then tot:ForceUpdate() end
end
...
function UI:Build()
    ...
    if not CFG() then
        print("|cffff4444[RobUI]|r ToT settings not ready.")
        return
    end

    local f = CreateFrame("Frame", "RobUI_TestToT_Settings", UIParent, "BackdropTemplate")
    self.frame = f
    f:SetSize(860, 820)
    f:SetPoint("CENTER")
    Skin(f, C_BG)
    ...
    title:SetText("RobUI Target-of-Target Settings (Shift+Drag window)")
    ...
    -- B. POSITION
    y = CreateSection(content, "Position (Shift + Left Drag ToT)", x, y, W)
    self.rows = {}
    ...
    local dd = CreateFrame("Frame", "RobUI_ToT_BaseTexDD", content, "UIDropDownMenuTemplate")
    self.baseTexDD = dd
    ...
    self.cbClass = Check(content, "Use Class Color (player ToT)", x+10, y-10, nil, function(v)
        local cfg = CFG()
        cfg.useClassColor = v
